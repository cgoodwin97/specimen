  ...
  try {
    ...
    try {
      const protRes = await fetch(
        `https://rest.uniprot.org/uniprotkb/search?query=(go:${goId}) AND (organism_id:9606) AND (reviewed:true)&fields=accession,id,gene_names,protein_name&format=json&size=6`,
        { headers: { Accept: 'application/json' } }
      );
      if (protRes.ok) {
