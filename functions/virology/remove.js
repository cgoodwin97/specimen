...
export async function onRequestPost({ request, env }) {
  ...
  try {
    const { taxon_id } = await request.json();

    if (!taxon_id) {
      return new Response(JSON.stringify({ ok: false, error: 'Missing taxon_id.' }), {
        status: 400, headers: corsHeaders('application/json', request),
      });
    }

    await env.SPECIMEN_DB.prepare(
      'DELETE FROM saved_viruses WHERE user_id = ? AND taxon_id = ?'
    ).bind(session.user_id, taxon_id).run();
