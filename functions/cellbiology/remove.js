...
export async function onRequestPost({ request, env }) {
  ...
  try {
    const { go_id } = await request.json();

    if (!go_id) {
      return new Response(JSON.stringify({ ok: false, error: 'Missing go_id.' }), {
        status: 400, headers: corsHeaders('application/json', request),
      });
    }

    await env.SPECIMEN_DB.prepare(
      'DELETE FROM saved_structures WHERE user_id = ? AND go_id = ?'
    ).bind(session.user_id, go_id).run();
